...
reframe(
    ...
    #Mode_Yield = as.numeric(names(table(`hg/ha_yield`)[which.max(table(`hg/ha_yield`))]))
...
trend_plot1 = data %>%
  ...
  labs(title = "Trend of Yield Over Years in United Kingdom", x = "Year", y = "Yield (hg/ha)") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
trend_plot2 = data %>%
  ...
  labs(title = "Trend of Yield Over Years in Botswana", x = "Year", y = "Yield (hg/ha)") +
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
# This will give you statistics such as AIC, BIC, and KS
# (Kolmogorov-Smirnov) for each distribution.
# Lower AIC and BIC values indicate a better fit.
# The KS statistic measures the maximum difference
# between the empirical distribution function of
# your data and the theoretical distribution. Smaller
# KS values suggest a better fit.
